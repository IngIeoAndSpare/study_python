...
def activation(z, derivative=False):
    ...
    # print(f"z ====> {z}")
    sigmoide_value = 1 / (1 + np.exp(-z))
    result = None

    if derivative:
    	###########################################################################
	    # TODO: Implement the derivative of sigmoid and return it           #
 		###########################################################################
        result = sigmoide_value * (1 - sigmoide_value)
	    ###########################################################################
	    #                             END OF YOUR CODE                            #
	    ###########################################################################
    else:
    	###########################################################################
	    # TODO: Implement the sigmoid and return it           #
 		###########################################################################
        result = sigmoide_value
	    ###########################################################################
	    #                             END OF YOUR CODE                            #
	    ###########################################################################
    return result
...
def cost_function(y_true, y_pred):
	...
	###########################################################################
    ## ref : https://medium.com/analytics-vidhya/deep-neural-networks-step-by-step-with-numpy-library-565836a867db
    m = y_pred.shape[1]
    cost = -1./m * np.sum(y_pred * np.log(y_true) + (1 - y_pred) * np.log(1- y_true))
...
class NeuralNetwork(object):     
    ...
    def forward(self, input):
        ...
        for index, (w, b) in enumerate(zip(self.weights, self.biases)):
		    ...
		    # a: output
		    # pre_activations: i.e. z value, 
		    # activations: values after the activation function#
			###########################################################################

            ## w*iput + bias all sum value
            pre_activations.append(np.dot(w, activations[index]) + b) 

            # calculate activation value
            a = activation(pre_activations[index])
            print(f"pre shape =>   {pre_activations[index].shape}")
            # append activation memory
            activations.append(a)
            ''' 
            weight_input_matmul = np.matmul(w, activations[0]).transpose()

            for item in weight_input_matmul:
                ## bias add
                pre_activations.append(item + b.transpose()[0])

            pre_activations.append(np.array(pre_activations).transpose()) 
            '''
		    ###########################################################################
		    #                             END OF YOUR CODE                            #
		    ###########################################################################
    ...
    def backpropagate(self, deltas, pre_activations, activations):
        ...
        db = []
        deltas = [0] + deltas
        # print(f"deltas    => {deltas[1].shape}")
        # print(f"activations => {activations[0].shape}")
    ...
    def train(self, X, y, batch_size, epochs, learning_rate, validation_split=0.2, print_every=10, tqdm_=True, plot_every=None):
        ...
        for e in epoch_iterator:
            ...
            for batch_x, batch_y in zip(batches_x, batches_y):
                print(f"batch x  => {batch_x}") # input
                print(f"batch y  => {batch_y}") # true value?

                batch_y_pred, pre_activations, activations = self.forward(batch_x)
                ## result, z value array(not sum), activation value (y value)

                ## whar dose mean about deltas??? it is loss value???
